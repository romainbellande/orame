...
use leptos::{wasm_bindgen::{prelude::*, JsCast}, leptos_dom::logging::console_log};
use ogame_core::coordinates;
use rand::Rng;
use uuid::timestamp::context;
use web_sys::HtmlCanvasElement;
use super::{utils::*, star::{StarBackground, StarColor, STAR_COLORS, self}};
...
impl Galaxy {
    pub fn new(canvas_id: String) -> Self {
        ...
        let distance_spread_max = if point_count < 20 {
          500
        } else {
          1000
        };
    ...
    pub fn distance(&self, coordinates: Coordinates) -> u16 {
      ...
      ((x - origin_x).powf(1.0) + (y - origin_y).powf(1.0)).sqrt().floor() as u16
    }

    pub fn get_radius(&self, center_distance: u16) -> u16 {
      let mut rng = rand::thread_rng();
      let distance = rng.gen_range(self.min_distance..self.max_distance);
      rng.gen_range(0..distance) + ((center_distance / self.max_distance_from_center) * self.distance_spread as u16)
    }
    ...
    pub fn spiral(&mut self, canvas: &mut web_sys::HtmlCanvasElement ) {
      ...
      for i in 0..=(self.point_count * 2) {
        ...
        let x = self.sign as f32 * self.log_spiral_coeff_1 as f32 * E.powf(self.log_spiral_coeff_2 * i) * i.cos();
      ...
      for i in 0..self.density {
        ...
        let bright_boost = if chance(Some(0.01)) {
          ...
          self.max_distance_from_center as f32 / (center_distance as f32 * self.bright_limiter)
        };
        ...
        let np = rotate(canvas_with / 2.0, canvas_height / 2.0, pos_x, pos_y, self.angle.clone());
        ...
        // draw
        if i == 0 {
          let radian = ctx.create_radial_gradient((canvas_with / 2.0).into(), (canvas_height / 2.0).into(), 0.0, (canvas_with / 2.0).into(), (canvas_height / 2.0).into(), self.core_size.into());

          if let Ok(radian) = radian {
            radian.add_color_stop(0.0, star_color.rgba(1.0).as_str()).expect("Failed to add color stop");
            radian.add_color_stop(1.0, star_color.rgba(0.0).as_str()).expect("Failed to add color stop");
            ctx.set_fill_style(&radian);
            ctx.fill_rect(0.0, 0.0, canvas_with.into(), canvas_height.into());
          }
        } else {
          if chance(Some(0.01)) {
            let radian = ctx.create_radial_gradient(pos_x.into(), pos_y.into(), 0.0, pos_x.into(), pos_y.into(), rng.gen_range(0.0..7.0));
            ...
            let radian = ctx.create_radial_gradient(pos_x.into(), pos_y.into(), 0.0, pos_x.into(), pos_y.into(), rng.gen_range(10.0..150.0));
            if let Ok(radian) = radian {
              radian.add_color_stop(0.0, star_color.get_with_overflow(0).rgba(0.05).as_str()).expect("Failed to add color stop");
              radian.add_color_stop(1.0, star_color.get_with_overflow(0).rgba(0.0).as_str()).expect("Failed to add color stop");
              ctx.set_fill_style(&radian);
              ctx.fill_rect(0.0, 0.0, canvas_with.into(), canvas_height.into());
            }
          } else {
            ...
            ctx.begin_path();
            ctx.arc(pos_x.into(), pos_y.into(), rng.gen_range(0.0..1.5), 0.0, 360.0).expect("Failed to arc");
            ctx.fill();
          }
        }
      }
    ...
    pub fn render(&mut self) {
      ...
      if self.back_drop {
        for _ in 1000..5000 {
          ...
          ctx.arc(pos_x, pos_y, rng.gen_range(0.0..0.1), 0.0, 360.0).expect("Failed to arc");
          ctx.fill();
        }
      }
